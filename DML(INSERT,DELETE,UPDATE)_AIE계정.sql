...
/*
    ...
    테아블의 모든 컬럼에 값을 직접 넣어 한 행을 넣고자 할때
    컬럼의 순서대로 VALUES에 값을 넣는다
    
    부족하게 값을 넣었을 때 => not enough value오류
    값을 더 많이 넣었을 때 => too many values오류
*/
INSERT INTO EMPLOYEE VALUES(300,'이시영','051117-1234567','hello@naver.com','01023456789'
                                    ,'D1','J5',500000,0.2,200,SYSDATE,NULL,DEFAULT);

INSERT INTO EMPLOYEE VALUES(300,'이시영','051117-1234567','hello@naver.com','01023456789'
                                    ,'D1','J5',500000,0.2,200,SYSDATE,NULL,DEFAULT,NULL);                                    
                                    
SELECT * FROM EMPLOYEE;

--------------------------------------------------------------------------------
/*
    2) INSERT INTO 테이블명(컬럼명,커럼명,컬럼명....)VALUEW(값,값,값....)
...
/*
    3)서브쿼리를 이용한 INSERT
    VALUES의 값을 직접 명시 하는 대신 서브쿼리로 조회된 결과값을 모두 INSERT 가능(여러행도 가능)
...
-- 부서코드가 'D1'이 사원들의 사번, 사원명, 부서코드, 입사일,사수번호 조회
...
SET SALARY = 400
    WHERE EMP_NAME = '박정보';
-- 조정연 사원의 급여르 410만원으로, 보너스를 0.25로 변경

-- 전체 사원의 급여를 기존 급여의 10%인상한 금액으로 변열 (기존급여 *1.1) = (기존급여 +(기존급여 * 0.1))
...
/*
    ...
    [WHERE 조건];
*/
-- 왕정보 사원의 급여와 보너스값으 ㄹ조정연사원의 급여와 보너스값으로 변경
UPDATE EMP_SALARY
...
--== JOIN으로 데이터 변경
--ASIA 지역에서 근무하는 지원들의 보너스를 0.3으로 변경
...
-------------------------------------------------------------------------------
--UPDATE시에도 해당 컬럼에 대한 제약조건에 위배되면 아됨
--사번이 200번인 사원의 이름 NULL변셩
UPDATE EMPLOYEE
SET EMP_NAME = NULL
WHERE EMP_ID = 200;

-- 완정보인 사원의 직급코드를 J9로 변경
UPDATE EMPLOYEE
SET JOB_CODE = 'D9'
...
/*
    *TRUNCATE :테이블의 전체 데이터르르 삭제할 때 사용하는 구문
    DELETE보다 수행속도가 빠르다
