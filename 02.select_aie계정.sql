...
SELECT*
FROM JOB;
--EMPLOYEE테이블에서 사번,이름,급여,입사일만 조회
...
--4. EMPLOYEE테이블에 사원면,미에일,전화번호,입사일,급여 조회+
...
--EMPLOYEE에서 사원명,급여,보너스,연봉,보너스를포함한연봉((급여+(보너스*급여))*12) 조회
SELECT EMP_NAME,SALARY,BONUS,SALARY*12,(SALARY+BONUS*SALARY)*12
FROM EMPLOYEE;
-- 산술연산중 NULL이 존재하면 결과는 무조건 NULL이된, 별도 처리(나중에)
...
FROM EMPLOYEE;
--함수수업시 DATE날짜러리하면 초단위를 관리할 수 있음
...
별칭에 띄어쓰기나 특수문자 포함되면 반드시(")쌍따옴표 넣어줘야 한다
*/

--EMPLOYEE에서 사원명,급여,보너스,연봉,보너스를포함한연봉((급여+(보너스*급여))*12) 조회
SELECT EMP_NAME 사원명,SALARY AS 급여,BONUS"보너스",SALARY*12"연봉(원)",(SALARY+BONUS*SALARY)*12AS"총 소득"
FROM EMPLOYEE;
...
조회된 RESELT SET의 모든 행에 반복적으로 출력
*/
--EMPLOYEE에 사번,사원명,급여,원 조회 AS단위
...
/*
<연결연산자: ||>
여러 컬럼들을 마치 하나의 컬럼값인것처럼 연결하거나, 멀럼값과 리터럴을 연결할 수 있음
*/
