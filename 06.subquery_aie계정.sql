...
AND SALARY < ANY(SELECT SALARY
                 ...
                 JOIN JOB USING(JOB_CODE)
                 WHERE JOB_NAME = '과장');
-- 4) 과장직급임에도 불구하고 차장직급인 사원들의 모든 급여보다 
    --   더 많이 받는 사원들의 사번, 사원명,직급명,  급여조회
...
AND SALARY > ANY(SELECT SALARY
               FROM EMPLOYEE
               JOIN JOB USING(JOB_CODE)
               WHERE JOB_NAME = '차장'); --280,155,249,248 차장들의 급여
-- ALL :서브쿼리의 값들 중 가장 큰 값보다 더 큰 값을 얻어올 때
--      비교대상 > 값 1 AND 비교대상 > 값 2 AND 비교대상 > 값 3
...
/*
    3. 다중열 서브쿼리 
       결과값이 한행이면서 여러 컬럼일 때
*/
-- 1) 장정보 사원과 같은 부서코드, 같은 직급코드 해당하는 사원들의 사번, 사원명, 부서코드, 직급코드 조회
...
/*
    ...
    서브쿼리 결과 여러행, 여러열일 경우
*/
-- 1) 각 직급별 최소 급여를 받는 사원의 사번, 이름 ,직급코드, 급여 조회
-- 1.1 각 직급별로 최소 급여를 받는 사원의 직급코드, 최소 급여 조회
...
WHERE (JOB_CODE,SALARY) IN ( SELECT JOB_CODE, MIN(SALARY)
                               FROM EMPLOYEE
                               GROUP BY JOB_CODE);
-- 2)각 부서 최고급여를 받는 사원들의 사번, 사원명, 부서코드, 급여 조회
...
FROM ( SELECT EMP_ID, EMP_NAME, SALARY*NVL((1+BONUS),1)*12 연봉, JOB_CODE
        FROM EMPLOYEE)
WHERE 연봉 >= 30000000;     
...
--어떻게 해주어야 하냐) 먼저 정렬을( ORDER BY) 한 테이블을 만들고
-- 그 테이블에서 ROWNUM을 부여해준다
...
SELECT ROWNUM, Y.*  --*를 사용 할때 테이블의 별칭을 정해주고 그 별칭에 모든것을
                    --가져온다는 의미로 별칭이Y면 Y.*을 사용해서 가져온다
FROM ( SELECT EMP_NAME, SALARY, DEPT_CODE
        FROM EMPLOYEE
        ORDER BY SALARY)Y
WHERE ROWNUM <= 5;
...
/*
    ...
    RNAK() OVER(정렬기준) | DENSE_RANK()OVER(정렬기준)
...
FROM EMPLOYEE
WHERE ROWNUM <= 5;
-- WHERE 순위 <= 5 실행 순서 때문에 오류
--WHERE RANK() OVER(ORDER BY SALARY DESC) <= 5;
-- RANK()함수는 SELECT 절에서만 사용 그래서 오류
