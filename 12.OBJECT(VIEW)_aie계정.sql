/*
    ...
    임시테이블 같은 존재(실제 데이터가 담겨있는건 아님 => 논리적 테이블)
*/
...
-- '일볼'에서 근무하는 사원들의 사번,사원명,부서명,급여,근무국가명 조회
SELECT EMP_ID,EMP_NAME,DEPT_TITLE,SALARY,NATIONAL_NAME
FROM EMPLOYEE
    JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
    JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
...
SELECT *
FROM(
    ELECT EMP_ID,EMP_NAME,DEPT_TITLE,SALARY,NATIONAL_NAME
    FROM EMPLOYEE
        JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
        JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
         JOIN NATIONAL USING(NATIONAL_CODE)
    );
-- 뷰는 논리적인 가상테이블(실직적으로는 데이터를 저장하고 있지않음)
SELECT *
    FROM VM_EMPLOYEE
WHERE NATIONAL_NAME = '한국';
...
*/
-- 전체사원읭 사번, 사원명, 직급명, 성별(남/여),근무년수를 조회할 수 있는 VIEW(VM_EMP_JOB생성)
...
-- 뷰를 통해 조작하ㅕㄴ 식제 데이터가 담겨있는 베이스 테이블에 반영됨
...
/*
    *단,DML명령어로 조작이 불가능한 경우가 더 많음
    ...
    2)뷰에 정의되어 있는 컬럼 중에 베니스 테이블 상에 NOT NULL제약조건이 지정되어있는 경우
    3)산술연삭식이나 함수식으로 정의되어있는 경우
...
DELETE 
    FROM VM_JOB
WHERE JOB_NAME ='사원';
-- 외래키가 되어있는 경우 자식테이즐에서 사용하고 있으면 삭제 안됨

--3)산술연삭식이나 함수식으로 정의되어있는 경우
CREATE OR REPLACE VIEW VM_EMP_SAL
AS SELECT EMP_ID,EMP_NAME,SALARY,SALARY*12연봉
    FROM EMPLOYEE;
    
-- INSERT(오류) -연봉은 기존 테이블에 없음
...
--4)그룹함수나 GROUP BY절이 포함되어 있는 경우
CREATE VIEW VM_GROUP_DEPT
AS SELECT DEPT_COD,SUM(SALARY)합계,CEIL(AVG(SALARY))
    FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
-- INSERT(오류)
INSERT INTO VM_JOIN VALUES(700,'황정민','총무부');
...
/*
    *VIEW 옵션
    ...
    1)OR REPLACE : 기존에 동일 뷰가 있으면 갱신시미코, 없으며 새로 생성
    2)FORCE|NOFORCE
...
-- 오류 : 서브쿼리 기술된 조건에 부합하지 않기 때문에 불가
UPDATE VM_EMP_CHECK
    SET SALARY =2000000
WHERE EMP_ID = 201;
