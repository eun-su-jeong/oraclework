...
/*
    ...
    1. 개체무결성 제약 조건 : NOT NULL, UNIQUW, CHECK(조건),PRIMARY KEY 조건위배
...
INSERT INTO MEMBER VALUES(1,'user01','1234','김나영','여','010-1234-5678','kim@naver.com','23/11/16');
INSERT INTO MEMBER VALUES(2,'user02','1234','박길남','남',null,NULL,'23/11/16');
--NOT NULL 제약조건에 위배되는 오류발생

--INSERT INTO MEMBER VALUES(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
INSERT INTO MEN_NOTNULL VALUES('2','user01','user02','pass01','이영순','여',null,null,sysdate);
INSERT INTO MEN_NOTNULL VALUES('1','user01','user02','pass01','이영순','여',null,null,sysdate);

-------------------------------------------------------------------------------

/*
    *UNIQUE 제약조건
    해당 컬럼 값에 중복된 값이 들어가면 안되는 경우
    컬럼값에 중복값 제한을 하는 제약조건
    삽입/수정 시 기존에 있는 데이터 값이 중복되었을 때 오류 발생
*/
...
/*
    UNIQUE(MEM_NO,MEM_ID)
    :MEN_NO 와 MEM_ID 의 조합한 결과값으로 중복을 체크함
    
    1. A -> DB에 데이터가 이렇게 들어있을때
    2. B
    3. C
...
INSERT INTO MEM_UNIQUE VALUES(1,'user02','pass02','김남길','남',null,null);

INSERT INTO MEM_UNIQUE VALUES(1,'user01','pass01','김남길','남',null,null);
INSERT INTO MEM_UNIQUE VALUES(2,'user01','pass02','김남길','남',null,null);
...
CREATE TABLE MEM_UNIQUE3(
  MEM_NO NUMBER CONSTRAINT MEMN0_NN NOT NULL CONSTRAINT NOUNIQUE UNIQUE,
  MEM_ID VARCHAR(20) NOT NULL CONSTRAINT IDUNIQUE UNIQUE,
...
INSERT INTO MEM_UNIQUE3 VALUES(1, 'uid2','upw2','김길',null); -- 제약조건의 바뀐 이름을 볼 수 있음

INSERT INTO MEM_UNIQUE3 VALUES(2, 'uid2','upw2','ㄱ',null);
INSERT INTO MEM_UNIQUE3 VALUES(2, 'uid2','upw2','ㄱ','M');
-- 성별 남여
-------------------------------------------------------------------------------
/*
    *CHECK(조건식)제약조건
...
INSERT INTO MEM_CHECK VALUES(2,'user02','pass02','이길동','F',NULL,NULL,SYSDATE);  --  CHECK 방식
INSERT INTO MEM_CHECK VALUES(2,'user02','pass02','이길동','여',NULL,NULL,SYSDATE);

-------------------------------------------------------------------------------
/*
    *PRIMARY KEY(기본미) 제약조건
    테이블에서 각 행들을 식별하기 위해 사용될 컬럼에 부여하는 제약조건(식별자 역할))
    
    EX) 회원번호, 학번, 사번, 예약번호, 운송장 번호 ....
    
    PRIMARY KEY제약조건을 부여하면 그 컬럼에 자동으로 NOT NULL + UNIQUE 제약조건을 의미
...
/*
    복합키 사용 예(어떤 사용자가 어떤 물품을 찜했는지 데이터르르 보관할 때)
    1 A
    1 B
    1 C
    2 A 
...
INSERT INTO MEM_GRADE VALUES(10,'특별회원');
...
EMAIL VARCHAR2(20),
MEM_DATE DATE,
GRADE_ID NUMBER -- 회원등급을 보관할 칼럼
);

INSERT INTO MEM VALUES(1,'user01','pass0','홍길동','남',NULL,NULL,SYSDATE,NULL);
INSERT INTO MEM VALUES(1,'user02','pass02','이길동','남',NULL,NULL,SYSDATE,10);
INSERT INTO MEM VALUES(1,'user03','pass03','여길동','여',NULL,NULL,SYSDATE,50);
-- 유효한 회원등급 번호가 아님에도 입력욈

-------------------------------------------------------------------------------
/*
    *FOREIGN KEY(외래키)제약조건
    다른테이블에 조재하는 값만 들어와야되는 특정 컬럼에 부여하는 제약조건
    --> 다른 테이블을 참조한다고 표현
    ...
    --> 참조할 컬럼이 PRIMARY KEY이면 생략가능(자동으로 PRIMARY KEY외 외래키를 맺음)
*/
...
INSERT INTO MEM VALUES(1,'user01','pass0','홍길동','남',NULL,NULL,SYSDATE,NULL);
INSERT INTO MEM VALUES(2,'user02','pass02','이길동','남',NULL,NULL,SYSDATE,10);
INSERT INTO MEM VALUES(3,'user04','pass02','여길녀','여',NULL,NULL,SYSDATE,20);
INSERT INTO MEM VALUES(4,'user03','pass03','여길동','여',NULL,NULL,SYSDATE,10);
...
INSERT INTO MEMBER2(2,'user02','p02',24,NULL,NULL,NULL);
INSERT INTO MEMBER2(3,'user03','p03',24,NULL,DEFAULT,DEFAULT);

--==============================================================================
/*
    ======================== aie 계정 =========================================
    <DUBQUEERY를 이용한 테이블 생성>
...
CREATE TABLE EMPLOYEE_COPY3
    AS SELECT EMP_ID,EMP_NAME,SALARY,BONUS
        FROM EMPLOYEE
        WHERE 1=0;   -- 테이블이 구조만 복사하고자 할 때 쓰이는 구문 (데이터값이 필요 없을때)
