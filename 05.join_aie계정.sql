/*
    ...
    JOIN은 크기 "오라클 전용 구문"과 'ANSI 구문" (ANSI = 미국 국립 표준협회)
                               [용어 정리]
            오라클 전용 구문          |          ANSI 구문
--------------------------------------------------------------------------------
              등가 조인              |        내부조인 (INNER JOIN) =>JOIN USING/ON
            (EQUAL JOIN)           |        자연조인(NATURE JOIN) => JOIN USING
--------------------------------------------------------------------------------
              포괄 조인              |        왼쪽조인(LEFT OUTER JOIN)
...
/*
    1. 등가조인(EQUAL JOIN)/내부조인(INNER JOIN)
       ...
       (=일치하는 값이 없으면 조회에서 제외가 됨)
*/ 
--오라클 전용 구문
/*
    FROM에 조회하고자 하는 테이블들을 나열(, 구분자료)
    WHERE 절에 매칭 시킬 컬럼(연결고리)에 대한 조건 제시함 
*/ 
...
--일치하는 값이 없는행은 조회에서 제외(NULL값은 제외)

-- 2)연결할 두 컬럼명이 같은 경우(EMPLOYEE:JOB_CODE, JOB:JOB_CODE)
-- 전체 사원들의 사번, 사원명, 직급코드, 직금명 조회
...
--해결방법1) 테이블에 별칭을 이용하는 방법       --별칭을 안넣고도 그냥 테이블 명을 넣어도 됨
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E
...
/*
    2.포괄조인 / 외부조인(OUTER JOIN)
      두 테이블간의 JOIN시 일치 하지 않는 행동 포함시켜 조회
      단, 반드시 LEFT/RIGHT를 지정해야됨(기준이 되는 테이블 지정)
      
*/
-- 내부 조인시 부서배치가 안된(NULL값) 사원2명은 정보가 나오지 않음
--사원명, 부서명, 급여, 연봉 조회
...
-- 3)FULL[OUTER] JOIN: 두 테이블 기술된 모든 행을 조회(단, 오라클 전용 구문이 없음)
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
